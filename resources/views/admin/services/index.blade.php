...
<style>
  ...
  <!-- The Modal -->
  <div class="modal fade" id="CreateServices">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <!-- Modal body -->
        <form  id="services_form" enctype="multipart/form-data"
                        method="POST">
...
<script src="https://cdn.ckeditor.com/4.20.1/standard/ckeditor.js"></script>
  ...
  <script>
	...
	var dataTable = $('.datatable').DataTable({
		...
		ajax: "{{ route('manage-services.index') }}",
		columns: [{
				data: 'id'
			},
			{
	...
	// Delete Services Ajax request.
	$(document).on("click", ".DeleteServices", function(e) {
		e.preventDefault();
		var id = $(this).data('id');
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "{{ url('admin/manage-services/') }}/" + id,
					method: 'DELETE',
	...
	$(document).on('click', '.EditServices', function() {
		...
		$.get("{{ route('manage-services.index') }}" + '/' + service_id + '/edit', function(data) {
	...
	$("#services_form").submit(function(e) {
		...
		submitHandler: function(form) {
			...
			let formData = new FormData(document.getElementById("services_form"));;
			$.ajax({
				url: "{{ route('manage-services.store') }}",
				type: "POST",
				data: formData,
				contentType: false,
				processData: false,
