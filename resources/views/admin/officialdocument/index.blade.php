...
<style>
                ...
                <div class="card" style="border-radius: 6px;">
                    <div class="card-body">
                        <div class="float-right mb-3">
                            ...
                            <button type="button" class="btn btn-primary AddOfficialDocument" >
                            <i class="fa fa-plus"></i> Create Official Document
                            </button>
                        </div>
  ...
  <!-- The Modal -->
  <div class="modal fade" id="CreateOfficialDocument">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
      
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">Create Official Document</h4>
...
<script src="https://cdn.ckeditor.com/4.20.1/standard/ckeditor.js"></script>
  ...
  <script>
	...
	var dataTable = $('.datatable').DataTable({
		...
		ajax: "{{ route('manage-official-document.index') }}",
		columns: [{
				data: 'id'
			},
			{
	...
	// Delete OfficialDocument Ajax request.
	$(document).on("click", ".DeleteOfficialDocument", function(e) {
		e.preventDefault();
		var id = $(this).data('id');
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "{{ url('admin/manage-official-document/') }}/" + id,
					method: 'DELETE',
	...
	$('button.btn.btn-primary.AddOfficialDocument').click(function() {
		...
		$('.modal-header h4.modal-title').html("Add official document");
		$('#saveBtn').val("add-official-document");
		$('#CreateOfficialDocument').modal('show');
		$('.form_button').text('add');
	});


	$(document).on('click', '.EditOfficialDocument', function() {

		var official_document_id = $(this).data('id');
		$.get("{{ route('manage-official-document.index') }}" + '/' + official_document_id + '/edit', function(data) {
			$('.modal-header h4.modal-title').html("Edit official document");
			$('#saveBtn').val("edit-official-document");
			$('#CreateOfficialDocument').modal('show');
			$('#official_document_id').val(data.id);
			$('#title').val(data.title);
			var editor = CKEDITOR.instances.description;
			editor.setData(data.description);
			$('img.previewImg.img-thumbnail').attr('src', data.image);
	...
	$("#official_documents_form").submit(function(e) {
		...
		submitHandler: function(form) {
			...
			$.ajax({
				url: "{{ route('manage-official-document.store') }}",
				type: "POST",
				data: formData,
				contentType: false,
				processData: false,
				success: function(result) {
					...
					if (result.error) {
						...
						toastr.success(result.success);
						$('#official_documents_form')[0].reset();
						$('#CreateOfficialDocument').modal('hide');
						$('.datatable').DataTable().ajax.reload();
					}
				}
			});
